...
import { Plus, Pencil, Trash2, QrCode, ScanLine, Search } from "lucide-react";
            ...
            <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setEditing(null); setForm(emptyForm); } }}>
              ...
              <DialogContent className="max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="grid gap-3 sm:grid-cols-2">
                  <div><Label>Kode Barang</Label><Input value={form.kode_barang} onChange={(e) => setForm({ ...form, kode_barang: e.target.value })} required /></div>
                  <div><Label>Nama Barang</Label><Input value={form.nama_barang} onChange={(e) => setForm({ ...form, nama_barang: e.target.value })} required /></div>
                  <div>
                  ...
                  <Button type="submit" disabled={loading} className="sm:col-span-2">{loading ? "Menyimpan..." : "Simpan"}</Button>
                </form>
              </DialogContent>
            </Dialog>
          )}
      ...
      <div className="rounded-md border overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {filtered.map((p) => (
              <TableRow key={p.id}>
                <TableCell className="font-mono text-sm">{p.kode_barang}</TableCell>
                <TableCell className="font-medium">{p.nama_barang}</TableCell>
                <TableCell>{p.categories?.nama ?? "-"}</TableCell>
                <TableCell>{p.stok} {p.satuan}</TableCell>
                <TableCell>{formatCurrency(p.harga_beli)}</TableCell>
                <TableCell>
        ...
        <DialogContent>
          ...
          {selectedProduct?.qr_code_url && (
            <div className="flex flex-col items-center gap-4">
              ...
              <Button asChild><a href={selectedProduct.qr_code_url} download={`qr-${selectedProduct.kode_barang}.png`}>Download QR</a></Button>
            </div>
          )}
        </DialogContent>
      </Dialog>
