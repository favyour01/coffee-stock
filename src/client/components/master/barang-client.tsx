...
export function BarangClient({ products, categories, suppliers, units }: BarangClientProps) {
  ...
  const columns = useMemo<DataTableColumn<Product>[]>(() => {
    const cols: DataTableColumn<Product>[] = [
      ...
      { id: "nama", header: "Nama", sortable: true, sortValue: (p) => p.nama_barang, cell: (p) => <span className="font-medium">{p.nama_barang}</span> },
      { id: "kategori", header: "Kategori", sortable: true, sortValue: (p) => p.kategori_nama ?? p.categories?.nama ?? "", cell: (p) => p.kategori_nama ?? p.categories?.nama ?? "-" },
      { id: "stok", header: "Stok", sortable: true, sortValue: (p) => p.stok, cell: (p) => `${p.stok} ${p.satuan}` },
      { id: "harga", header: "Harga Beli", sortable: true, sortValue: (p) => p.harga_beli, cell: (p) => formatCurrency(p.harga_beli) },
      { id: "status", header: "Status", cell: (p) => p.stok < p.minimum_stok ? <Badge variant="destructive">Stok Menipis</Badge> : <Badge variant="secondary">Aman</Badge> },
    ];

    if (canEdit) {
      cols.push({
        id: "aksi", header: "Aksi", headerClassName: "w-32",
        cell: (p) => (
          <div className="flex gap-1">
            <Button variant="ghost" size="icon" onClick={() => handleEdit(p)}><Pencil className="h-4 w-4" /></Button>
            <Button variant="ghost" size="icon" onClick={() => {
              if (p.qr_code_url) { setSelectedProduct(p); setQrOpen(true); }
              else handleGenerateQR(p);
            }}><QrCode className="h-4 w-4" /></Button>
            <Button variant="ghost" size="icon" onClick={async () => {
              if (!confirm("Hapus barang?")) return;
              try { await productApi.delete(p.id); toast.success("Dihapus"); qc.invalidateQueries({ queryKey: ["products"] }); }
    ...
    <div className="space-y-6">
                    ...
                    <Field label="Satuan">
                      {units.length > 0 ? (
                        <Select value={form.satuan} onValueChange={(v) => setForm({ ...form, satuan: v })}>
                          ...
                          <SelectContent>{units.map((u) => <SelectItem key={u.id} value={u.singkatan}>{u.nama} ({u.singkatan})</SelectItem>)}</SelectContent>
                        </Select>
                      ) : (
      ...
      <DataTable
        ...
        searchFilter={(p, q) =>
          ...
          (p.kategori_nama ?? p.categories?.nama ?? "").toLowerCase().includes(q)
        }
          ...
          <div className="px-6 pb-6">
            {selectedProduct?.qr_code_url && (
              <div className="flex flex-col items-center gap-4">
                <img src={selectedProduct.qr_code_url} alt="QR Code" className="h-64 w-64 object-contain" />
                <p className="text-sm text-muted-foreground">{selectedProduct.kode_barang}</p>
                <Button asChild>
                  <a href={selectedProduct.qr_code_url} download={`qr-${selectedProduct.kode_barang}.png`}>Download QR</a>
                </Button>
              </div>
            )}
          </div>
        ...
        <DialogContent className="p-0 sm:max-w-md">
          ...
          <div className="px-6 pb-6">
            <BarcodeScanner onScan={(code) => {
              ...
              if (found) { toast.success(`Ditemukan: ${found.nama_barang} (Stok: ${found.stok} ${found.satuan})`); setScanOpen(false); }
              else toast.error(`Barang dengan kode ${code} tidak ditemukan`);
            }} />
          </div>
        </DialogContent>
      </Dialog>
